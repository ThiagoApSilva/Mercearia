...
if($forma_pgto_input != ""){
	...
	$res = $pdo->prepare("INSERT INTO vendas SET valor = :valor, data = curDate(), hora = curTime(),  operador = :usuario, valor_recebido = :valor_recebido, nome = :nome, CPF = :cpf, desconto = :desconto, troco = :troco, forma_pgto = :forma_pgto, abertura = :abertura, status = 'Concluída' ");
	...
	$res->bindValue(":nome", $nome_input);
	$res->bindValue(":cpf", $cpf_input);
	$res->bindValue(":forma_pgto", $forma_pgto_input);
	$res->bindValue(":abertura", "1");
	$res->execute();
	...
	if($forma_pgto_input == 4){
		...
		if($cpf_confirmado == "sim"){
			$total = $valor - $desconto + $total_compra;
			$query_con = $pdo->query("UPDATE contas_em_aberto SET total_conta = '$total'  WHERE CPF = '$cpf_input'");
			...
			$res2 = $pdo->prepare("INSERT INTO pagamento SET dt_pagamento = curDate(), pagamento = :pagamento, nome = :nome, CPF = :cpf");
			$res2->bindValue(":pagamento", $desconto);
			$res2->bindValue(":nome", $nome_input);
			$res2->bindValue(":cpf", $cpf_input);
			$res2->execute();
